...
from helpers.image import PPTextBox, ScoreBox, StarRatingTextBox, TitleTextBox, DifficultyTextBox, \
...
class ScoreCard:
    def __init__(self, parser: Parser, scores: List[Score]):
        self.parser = parser
        self.scores = scores
...
class ImageScoreCard(ScoreCard, ABC):
    ...
    async def draw_image(self, score: Score):
        """
...
class SingleImageScoreCard(ImageScoreCard, ABC):
    ...
    async def draw_image(self, score: Score):
        cover_image_path = await self.download_and_save_asset(score.beatmapset.covers.card_2x)
        beatmap_path = await self.download_and_save_beatmap(score.beatmap.id)
        calculator = Calculator(beatmap_path)
        [rosupp_result] = calculator.calculate(ScoreParams(mods=score.mods.value,
        ...
        title = TitleTextBox(beatmap.beatmapset().title)
        ...
        ModsIcon(score.mods).draw(cover, Point(cover.width - right_offset + 80, 110))
        ScoreGradeVisual(score.rank).draw(cover_draw, Point(cover.width - right_offset + 150, 30))
